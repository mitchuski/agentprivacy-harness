...
//   2. restructurer: a further structure-level pass (merges, densification)
//      of the folded text — fact-moving, so expect probe-limited discipline.
...
export default {
  ...
  finders: [
    {
      lens: 'line-editor',
      hint: 'COMPOSED PASS (OT-3). Your source text is artifact/GUIDE.compressed.md — the folded frontier-best candidate — NOT the original guide. Apply sentence-level compression over it: cut residual redundancy, filler, connective tissue, and free-standing punctuation tokens; tighten phrasing. NEVER delete a fact — every quantity, duration, temperature, item, and ordered step must survive, because the gate draws from the ORIGINAL guide regardless of your source text.',
    },
    {
      lens: 'restructurer',
      hint: 'STRUCTURE PASS (OT-3). Your source text is artifact/GUIDE.compressed.md — the folded frontier-best candidate. Apply structure-level compression over it: merge sections that share context, collapse bullets that restate their heading, convert residual prose to denser list/table forms. This lens MOVES facts, which is where mirages live: NEVER delete a fact — every quantity, duration, temperature, item, and ordered step in the ORIGINAL guide must survive relocation, because the gate draws from the original.',
    },
  ],
...
Propose exactly 1 lever through YOUR lens: a complete rewrite of the folded text, below the frontier best. leverId = "${finder.lens === 'line-editor' ? 'r4-compose-line-edit' : 'r4-compose-restructure'}". Your proposal's compressedText field MUST carry the full candidate text (this is the artifact the Gap will hash). State expectedMetric = its word count by the counting rule, and hardConstraintNote = why it is still a self-contained instruction document. Plan and write text only — never touch files.`,
...
Round context you must weigh: BOTH levers compose over the folded 526-word best (which passed two disjoint draws AND an exhaustive 32/32 fact census — CR-4). The line-edit lens on already-telegraphic text may hit diminishing returns: judge whether its gain is draw-invariant by construction or whether the text is near its fact-density floor. The restructure lens MOVES facts over an already-reshaped text — the classic probe-limited shape; say what would convert it. If either drops below ~490 the critic's r3 projection is met; if neither validates, say plainly whether OT-3 should be revised or the lever family killed (GR-6, with a re-open condition). Draft KILLED_LEVERS entries for structural kills. Name exactly ONE next lead.`,
